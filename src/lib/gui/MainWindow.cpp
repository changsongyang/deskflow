...
void MainWindow::updateNetworkInfo()
{
  ...
  if (suggestedAddress.isEmpty()) {
    ...
    ui->lblIpAddresses->setToolTip(tr("Unable to detect an IP address. Check that your network connection is active"));
    return;
  }

  ui->lblIpAddresses->setText(QStringLiteral("Suggested Ip: %1").arg(suggestedAddress));

  if (ipList.count() < 2) {
    ui->lblIpAddresses->setToolTip("");
  } else {
    ui->lblIpAddresses->setToolTip(
        tr("<p>If connecting via the hostname fails, Try the following Ips:<br/>%1</p>").arg(ipList.join("<br/>"))
    );
  }
}
